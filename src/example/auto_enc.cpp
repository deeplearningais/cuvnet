...
struct auto_encoder{
    ...
    auto_encoder(unsigned int inp0, unsigned int inp1, unsigned int hl, float noise=0.0f, float lambda=0.0f)
    ...
    ,m_bias_y(new Input(cuv::extents[inp1],     "bias_y"))
    {
        ...
        // negative cross-entropy loss of logistic (stable/fast version)
        //m_rec_loss    = mean( sum(neg_log_cross_entropy_of_logistic(m_input,m_decode),1)); // reconstruction loss (cross-entropy)
