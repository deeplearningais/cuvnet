...
namespace cuvnet
{
	...
	bool inBall = true;
        ...
        if(axis == 1){
	    if (logspace){
                cuv::reduce_to_row(ax, C, cuv::RF_LOGADDEXP);
	        cuv::apply_scalar_functor(ax, ax, cuv::SF_EXP);
	        if (inBall)
		     cuv::apply_scalar_functor(ax, ax, cuv::SF_SQUARE);
	    }
            else {
		if ( inBall)
	            cuv::reduce_to_col(ax, C, cuv::RF_ADD_SQUARED);
	        else
                    cuv::reduce_to_col(ax, C, cuv::RF_ADD);
	    }
        }
        else if(axis == 0){
	    if (logspace){
                cuv::reduce_to_col(ax, C, cuv::RF_LOGADDEXP);
	        cuv::apply_scalar_functor(ax, ax, cuv::SF_EXP);
	        if (inBall)
		       	cuv::apply_scalar_functor(ax, ax, cuv::SF_SQUARE);
	    } else {
        ...
        cuv::apply_scalar_functor(ax, cuv::SF_ADD , 1.f, &over_thresh); // ax[!over_thresh] += 1
      	...
      	if(axis == 1){
            if ( logspace ){
	        cuv::apply_scalar_functor(ax, ax, cuv::SF_LOG);
	        cuv::apply_scalar_functor(ax, ax, cuv::SF_NEGATE);
                cuv::matrix_plus_row(C, ax);
	    } else
                cuv::matrix_divide_row(C, ax);
	}
        else if(axis == 0){
