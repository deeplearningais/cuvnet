...
namespace cuvnet
{
    ...
    inline
        boost::shared_ptr<Spn_Output_Op> spn_output_op(Op::op_ptr img, Op::op_ptr m_W, Op::op_ptr Y, boost::shared_ptr<cuvnet::monitor> S, unsigned int classes, float eps = 0.00001f) { return boost::make_shared<Spn_Output_Op>(img->result(), m_W->result(), Y->result(), S, classes, eps); }
